...
# These two are published once a day by staleness.yml, not regenerated by
# this rebuild - carry the last-published copy forward so every 6-hourly/
# webhook orphan rebuild doesn't blank the status dashboard until the next
# daily run. Best-effort: a missing/failed fetch just leaves the dashboard
# showing its previous cached copy client-side, nothing here depends on it.
carry_forward_status_reports() {
  local f
  for f in dists/staleness.json dists/parity.json; do
    curl -fsSL --connect-timeout 10 --max-time 30 \
      -o "$WORK/gh-pages/$f" \
      "https://raw.githubusercontent.com/${GITHUB_REPOSITORY}/gh-pages/$f" \
      || rm -f "$WORK/gh-pages/$f"
  done
}
